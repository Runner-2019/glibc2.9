...
#              else /* just guess */
#                define malloc_getpagesize (4096) // 记住这一句就可以
#              endif
#            endif
#          endif
#        endif
...
// 如果没有定义MAP_ANONYMOUS, 我们就借助/dev/zero
...
// 32位下，size_t == 4， 所以MIN_CHUNK_SIZE == 4 * size_t, 注意是包括fd的
#define MIN_CHUNK_SIZE        (offsetof(struct malloc_chunk, fd_nextsize))

/* The smallest size we can malloc is an aligned minimal chunk */
// MIN_CHUNK_SIZE地址对齐之后的值为MINSIZE
...
// 这三个宏用于将用户请求的分配大小转换成内部需要分配的 chunk 大小，

// 注意负数转化成 unsigned long，会表示一个最大的整数，以此来判断out-of-range
#define REQUEST_OUT_OF_RANGE(req)                                 \
  ((unsigned long)(req) >=                                        \
   (unsigned long)(INTERNAL_SIZE_T)(-2 * MINSIZE))

/* pad request bytes into a usable size -- internal version */
// 考虑 （负载头部，地址对齐），其实可以看出有个负载是 SIZE_SZ, 对齐如下
// 注意：负载是SIZE_SZ，而不是2 * SIZE_SZ，因为chunk复用了一个prev_size，看我们的笔记
...
// 这里减去一个 offsetof(malloc_chunk, fd)的意义何在
#define bin_at(m, i) \
...
// 从small bin里 取下来很简单，
// 如果还在large bin里，那就还要调整fd_nextsize 和 bk_nextsize
// 宏 unlink(P, BK, FD)用于将 chunk 从所在的空闲链表中取出来，注意 large bins 中的空闲
// chunk 可能处于两个双向循环链表中，unlink 时需要从两个链表中都删除。

// 何时，FD->fd_nextsize为nullptr？？？？
...
// 这里要看清楚，FASTCHUNKS_BIT == 0时，表示有FAST_CHUNKS
...
//mp_是全局唯一的一个 malloc_par 实例，用于管理参数和统计信息，

static struct malloc_par mp_;


/* Maximum size of memory handled in fastbins.  */
// 表示 fast bins 中最大的 chunk 大小
...
/*
    初始化函数做的工作比较简单，首先遍历所有的 bins，初始化每个 bin 的空闲链
    表为空，即将 bin 的 fb 和 bk 都指向 bin 本身。由于 av 中所有字段默认为 0，即默认分配连
    续的虚拟地址空间，但只有主分配区才能分配连续的虚拟地址空间，所以对于非主分配区，
    需要设置为分配非连续虚拟地址空间。如果初始化的是主分配区，需要设置 fast bins 中最大
    chunk 大小，由于主分配区只有一个，并且一定是最先初始化，这就保证了对全局变量
...
/* Forward declarations.  */
static Void_t *malloc_hook_ini
__MALLOC_P ((size_t
sz,
const __malloc_ptr_t caller
));
static Void_t *realloc_hook_ini
__MALLOC_P ((
Void_t *ptr, size_t
sz,
const __malloc_ptr_t caller
));
static Void_t *memalign_hook_ini
__MALLOC_P ((size_t
alignment,
size_t sz,
const __malloc_ptr_t caller
...
#endif
{
    ...
    */
    ...
    // 所请求的内存大小可以用mmap函数来分配，同时没有超过已分配的最大值
    if ((unsigned long) (nb) >= (unsigned long) (mp_.mmap_threshold) &&
        ...
        */
        ...
        /*
                如果重新计算所需分配的 size 小于 nb，表示溢出了，不分配内存，否则，调用 mmap()
                分配所需大小的内存。如果 mmap()分配内存成功，将 mmap()返回的内存指针强制转换为
                chunk 指针，并设置该 chunk 的大小为 size，同时设置该 chunk 的 IS_MMAPPED 标志位，表
                示本 chunk 是通过 mmap()函数直接从系统分配的。由于 mmap()返回的内存地址是按照页对
    ...
    */
    ...
    /*
        检查 top chunk 的合法性，如果第一次调用本函数，top chunk 可能没有初始化，可能
        old_size 为 0，如果 top chunk 已经初始化，则 top chunk 的大小必须大于等于 MINSIZE，因
        为 top chunk 中包含了 fencepost，fencepost 需要 MINSIZE 大小的内存。Top chunk 必须标识
        前一个 chunk 处于 inuse 状态，这是规定，并且 top chunk 的结束地址必定是页对齐的。另
        外 top chunk 的除去 fencepost 的大小必定小于所需 chunk 的大小，不然在_int_malloc()函数
        中就应该使用 top chunk 获得所需的 chunk。，由于_int_malloc()在调用本函数前，已经将 fast bins 中的所有 chunk
        都合并加入到 unsorted bin 中了，所以，本函数中 fast bins 中一定不会有空闲 chunk 存在。
     */
        ...
        */
        ...
        if (brk != (char *) (MORECORE_FAILURE)) {
            ...
            */
            ...
            if (contiguous(av)) // 加回来刚才减少的old_size
            ...
            if ((unsigned long) (size) > (unsigned long) (nb)) {
                ...
                if (mbrk != MAP_FAILED) {
                    ...
                    */
                    ...
                    //如果所需分配的内存大小合法，使用 mmap()函数分配内存。如果分配成功，
                    // 更新 brk 和 snd_brk，并将当前分配区属性设置为可分配不连续虚拟内存块。
                    set_noncontiguous(av);
                }
            }
#endif
        ...
        /*
         * 如果brk合法，即sbrk()或mmap()分配成功，
         * 如果sbrk_base还没有初始化，更新sbrk_base和当前分配区的内存分配总量。
         */
        if (brk != (char *) (MORECORE_FAILURE)) {
            if (mp_.sbrk_base == 0)
                mp_.sbrk_base = brk;
            ...
            */
            ...
            /*
             * 如果 sbrk()分配成功，更新 top chunk 的大小，
             * 并设定 top chunk 的前一个 chunk 处于 inuse状态。
             * 如果当前分配区可分配连续虚拟内存，原 top chunk 的大小大于 0，
             * 但新的 brk 值小于原 top chunk 的结束地址，出错了。
             */
...
public_mALLOc(size_t bytes) {
    ...
    __malloc_ptr_t(*hook)(size_t,
    __const __malloc_ptr_t) = __malloc_hook;
    if (hook != NULL)
...
public_rEALLOc(Void_t *oldmem, size_t bytes) {
    ...
    __malloc_ptr_t(*hook)(__malloc_ptr_t, size_t,
...
_int_malloc(mstate av, size_t bytes) {
    ...
    */
    ...
    // 所请求的size小于当前最大的fastbin, 则可从fastbin中分配
    if ((unsigned long) (nb) <= (unsigned long) (get_max_fast ())) {
        long int idx = fastbin_index(nb); // 获得index
        fb = &(av->fastbins[idx]); //fastbin[idx] 不就是地址吗，为什么还要取&
        // // 读取chunk 这里使用的是地址的地址 方便下面移动fd指针的时候 就将 victim 从链表中移除
        if ((victim = *fb) != 0) { // 可以分配,fastbin是单链表，判断不空用的是nullptr
            // 这个是为什么呢？？
    ...
    */
    ...
    if (in_smallbin_range(nb)) { // 没有达到large bin的门槛，还在small bin的范围内
        idx = smallbin_index(nb); // 获得逻辑下标
        bin = bin_at(av, idx); // 找到实际的bin

        if ((victim = last(bin)) != bin) { // small bin是双链表，判断不空用的是 != header
            // 为何这里还有可能为 0 啊？
            if (victim == 0) /* initialization check */
                ...
                // 取下来victim
                bck = victim->bk;
                set_inuse_bit_at_offset(victim, nb); // small bin的处理，需要设置这个位
                bin->bk = bck;
                bck->fd = bin;
    ...
    for (;;) {
        ...
        while ((victim = unsorted_chunks(av)->bk) != unsorted_chunks(av)) {
            ...
            */
            ...
            // 符合这4个条件，从last_chunk_reminder中分配
            if (in_smallbin_range(nb) &&
                bck == unsorted_chunks(av) &&
            victim == av->last_remainder &&
            (unsigned long) (size) > (unsigned long) (nb + MINSIZE)) {
                // 分配合适的p容易，分配完后，我们还要妥善处置遗留下来的remainder

                /* split and reattach remainder */

                // size == victim->size == remainder_size[旧]
                remainder_size = size - nb; // 分配后，剩余的remainder size
                remainder = chunk_at_offset(victim, nb); //
                unsorted_chunks(av)->bk = unsorted_chunks(av)->fd = remainder;// unsorted_chunks(av) == bck
            ...
            // 从下面开始，我们在unsorted bin中不分配，只放回
            // 期待在下一轮循环中，再分配
            if (in_smallbin_range(size)) { // 放回small bin, if-else后统一放入
                victim_index = smallbin_index(size);
                bck = bin_at(av, victim_index);
                fwd = bck->fd;
            } else {
                victim_index = largebin_index(size); // 放回largebin
                bck = bin_at(av, victim_index);
                fwd = bck->fd;

                /* maintain large bins in sorted order */
                if (fwd != bck) { // 至少有两个largebin
                    ...
                    if ((unsigned long) (size) < (unsigned long) (bck->bk->size)) {
                        ...
                        // 画一下示意图吧，这里就是把victim插入到对应的largebin里去了
                        // 插入到了fwd->fd和fwd->fd->bk_nextsize中间
                        // 完全没必要按照它如下的方式来强行背，我们能写出来这个双链表插入就是可以的，背有屁用
                        victim->fd_nextsize = fwd->fd;
                        victim->bk_nextsize = fwd->fd->bk_nextsize;
                        fwd->fd->bk_nextsize = victim->bk_nextsize->fd_nextsize = victim;
                    } else {
                        assert((fwd->size & NON_MAIN_ARENA) == 0);
                        // 这里可以看出，largebin是升序排列的
                ...
                } else // 1个large bin chunk都没有的时候，自己插自己
                    victim->fd_nextsize = victim->bk_nextsize = victim;
            }

            mark_bin(av, victim_index);
            victim->bk = bck; // 一个常见的双链表的插入
            victim->fd = fwd;
            fwd->bk = victim;
            bck->fd = victim;
        ...
        */
        ...
        if (!in_smallbin_range(nb)) {
            ...
            if ((victim = first(bin)) != bin &&
                ...
                // bk_nextsize 的遍历 ？？？？？？？？？？？？
                // 这里没看懂，没找到意义何在，bk_nextsize不应该恰好是第一个吗，怎么还需要victim->fd
      ...
      bordering top into fastbins
        ...
        */
        ...
        if ((unsigned long) (size) >= FASTBIN_CONSOLIDATION_THRESHOLD) {
            ...
            if (av == &main_arena) { // 主分配区使用sYSTRIm 收缩内存
...
#endif
{
    ...
    // 本函数目的：将 fast bin 放入 unsorted bin
    // 使用fast bin之前肯定要判断有没有fast bin
    // 外层循环处理所有的fast bin
    // 内层循环处理 当前fast bin的每一个chunk
    // 对于每一个chunk ，判断bk和fd是否空闲，空闲需合并
    // 最后将合并后的加入unsorted bin
    ...
    */
    ...
    // 为什么max_fast == 0, 我们就知道了av还没有被初始化？
    if (get_max_fast () != 0) { // 这里是已经被初始化了，如果没有被初始化我们就初始化，然后返回
        /*
         * 注意到arena有一个flag字段，专门给fastbin用，
         * 用来标识是否有fastbin chunks
         */
        clear_fastchunks(av); // 清除掉fastbin的比特位，因为fastbin内的所有chunk要被我们合并了

        unsorted_bin = unsorted_chunks(av); // 索引到unsorted bin的头部
...
#if 0
        /*
         * 没太看懂，反正就是不要用get_max_fast()作为循环的终点
         */
...
#endif
        fb = &(av->fastbins[0]); // 从fastbin的下标0开始合并
        do {
            if ((p = *fb) != 0) {  // 当前fastbin chunk指针不为空，有很多个fastbin呢别忘了, 这里判断p != 0 是必要的
                *fb = 0;

                do { // 内层循环遍历bin中所有chunk
                    check_inuse_chunk(av, p); // 定义了malloc_debugs时才会执行

                    // 注意：fast bin中的chunk一定都是freed，所以我们可以放心的使用fd字段
                    // （ chunk + somesize (物理位置上相邻的chunk)，不一定是freed ）
                    nextp = p->fd;
                    ...
                    /*
                     * 如果当前 chunk 的前一个 chunk 空闲，则将当前 chunk 与前一个 chunk 合并成一个空闲chunk，
                     * 由于前一个 chunk 空闲，则当前 chunk 的 prev_size 保存了前一个 chunk 的大小，
                     * 计算出合并后的 chunk 大小，并获取前一个 chunk 的指针，将前一个 chunk 从空闲链表中删除
                     */

                    if (!prev_inuse(p)) { // 物理位置相邻的前一个chunk是freed，合并前一个
                        prevsize = p->prev_size; // 这两个size操作，包括了headers了吗？
                        size += prevsize;
                        p = chunk_at_offset(p, -((long) prevsize)); // 注意：这里的p有了新的含义
                        unlink(p, bck, fwd); // 注意，因为此时的p不是先前的p，unlink的实际是prev chunk
                    }

                    // 注意：因为涉及到合并，所以我们要将物理位置位置上相邻的chunk从它们自己所在的那个bin中
                    // 先取下来，这个bin我们并不知道是哪个，通过unlink宏实现的

                    // 刚才已经取下来物理位置相邻的前一个了，并且取下来了，现在我们来处理物理位置相邻的下一个
                    // 注意这里的p可能已经更换了含义，p可能变成了prev chunk，

                    if (nextchunk != av->top) { // 下一个chunk 不是top chunk，将合并后的chunk插入到unsorted bin中
                        nextinuse = inuse_bit_at_offset(nextchunk, nextsize);

                        /*
                         * 如果与当前 chunk 相邻的下一个 chunk 处于 inuse 状态，
                         * 清除当前 chunk 的 inuse 状态，则当前 chunk 空闲了。否则，
                         * 将相邻的下一个空闲 chunk 从空闲链表中删除，
                         * 并计算当前chunk 与下一个 chunk 合并后的 chunk 大小。
                         *
                         */

                        // 下一个物理相连的chunk是freed。
                        if (!nextinuse) {  // 此时前面如果是free，p已经被从fast bin取下来了，如果不是free，p没动
                            size += nextsize;
                            unlink(nextchunk, bck, fwd);// p->prev chunk 可能已经被取下来了，我们只取p->next chunk
                        } else // nextChunk正在使用中，不需要合并了，我们就设置一下nextChunk的P位就好，因为此前是0，现在要设置为1
                            clear_inuse_bit_at_offset(nextchunk, 0);

                        first_unsorted = unsorted_bin->fd; // 插入操作
                        unsorted_bin->fd = p; // 将当前的p加入到unsorted_bin
                        first_unsorted->bk = p;

                        // 从fast bin转移到large bin时，要清除fd_nextsize标志
                        // 因为在 unsorted bin 中这两个字段无用
                        if (!in_smallbin_range (size)) { // 当前不在small bin的范围内了，那么fd_nextsize一定是被初始化了
                            p->fd_nextsize = NULL; //
                            p->bk_nextsize = NULL;
                        }

                        set_head(p, size | PREV_INUSE); // 填充size字段和flag
                        p->bk = unsorted_bin;
                        p->fd = first_unsorted;
                        set_foot(p, size); // foot == 下一个chunk个的prev_size字段
                    } else {// 如果下一个chunk和top chunk挨着，
                        //    重新设置top chunk的起始位置
                        // top chunk是未使用的，所以
                        // next chunk是top chunk时，next chunk一定是未使用的
                        // 因此这里我们需要判断一下 nextinuse 比特位了
                        size += nextsize;
                        set_head(p, size | PREV_INUSE); // 为什么加上inuse位？
                        av->top = p; // 把top chunk指针移动一下就可以
                    }
